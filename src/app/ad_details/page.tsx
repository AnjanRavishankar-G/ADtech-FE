...
const fetchCampaignData = async (startDate: string | null, endDate: string | null, campaignId: string | null) => {
  try {
    ...
    <Layout>
      <div className="p-5">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          {/* Campaign Button */}
          <Link
            href={`/campaign?brand=${encodeURIComponent(selectedBrand || '')}&campaignId=${selectedCampaignId || ''}`}
            className="text-blue-600 bg-blue-50 shadow-md hover:bg-blue-100 focus:ring-2 focus:ring-blue-300 
            font-medium rounded-lg text-sm px-6 py-2.5 transition-all duration-200 ease-in-out
            dark:bg-blue-900/20 dark:text-blue-300 dark:hover:bg-blue-900/40 dark:shadow-lg"
          >
        ...
        <div className="shadow-2xl p-4 bg-white rounded-2xl overflow-x-auto max-h-96 dark:bg-black mt-4">
          <Table className="border border-default-100 rounded-lg">
            <TableHeader className="bg-black text-white top-0 z-10">
              <TableRow>
                <TableHead className="text-center">Ad Group Name</TableHead>
                ...
                <TableHead className="text-center">DPV</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody className="text-white">
              {campaignData.map((adGroup) => (
                <TableRow key={adGroup.SN} className="text-center">
                  ...
                  <TableCell className="border border-default-300">{adGroup.Type}</TableCell>
                  <TableCell className="border border-default-300">{adGroup.Clicks.toLocaleString()}</TableCell>
                  <TableCell className="border border-default-300">{adGroup.Impressions.toLocaleString()}</TableCell>
                  <TableCell className="border border-default-300">
                  ...
                  <TableCell className="border border-default-300">{adGroup.DPV.toLocaleString()}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        ...
        <div className="mt-12 flex gap-4 rounded-2xl">
          ...
          <div className="w-1/2 shadow-2xl p-4 bg-white rounded-lg dark:bg-black dark:text-white dark:shadow-[-20px_-10px_30px_6px_rgba(0,0,0,0.1),_15px_10px_30px_6px_rgba(45,78,255,0.15)]">
            <h2 className="text-2xl font-bold mb-6 text-center">Top 5 Ad Groups by Sales</h2>
                ...
                <TableBody>
                  {topSales.map((adGroup) => (
                    <TableRow key={adGroup.SN}>
                      <TableCell className="w-1/2">{adGroup.AdGroupName}</TableCell>
                      <TableCell className="w-1/2">{adGroup.Sales?.toLocaleString('en-IN', {
                        style: 'currency',
                        currency: 'INR',
                        minimumFractionDigits: 2,
                      }) || '-'}</TableCell>
          ...
          <div className="w-1/2 shadow-2xl p-4 bg-white rounded-lg dark:bg-black dark:text-white dark:shadow-[-10px_-10px_30px_4px_rgba(0,0,0,0.1),_10px_10px_30px_4px_rgba(45,78,255,0.15)]">
            <h2 className="text-2xl font-bold mb-6 text-center">Top 5 Ad Groups by Spend</h2>
            {/* Visualization after title */}
            <div className="h-[400px] mb-6">
              <HorizontalBarChart 
                series={spendSeries}
              ...
              <Table className="min-w-full border text-center">
                ...
                <TableBody>
                  {topSpend.map((adGroup) => (
                    <TableRow key={adGroup.SN}>
                      <TableCell className="w-1/2">{adGroup.AdGroupName}</TableCell>
                      <TableCell className="w-1/2">{adGroup.Spend?.toLocaleString('en-IN', {
                        style: 'currency',
                        currency: 'INR',
                        minimumFractionDigits: 2,
                      }) || '-'}</TableCell>
