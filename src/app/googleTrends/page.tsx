...
async function fetchInterestOverTime(keywords: string[]) {
    try {
        ...
        if (!initResponse.ok) {
            throw new Error(
                `Failed to initiate request: ${initResponse.status}`
            );
        }
...
export default function GoogleTrendsDashboard() {
    ...
    const handleSearch = async () => {
        if (inputKeyword.trim()) {
            ...
            try {
                ...
                const originalKeywords = inputKeyword
                    .split(",")
                    .map((k) => k.trim());
                ...
                return (
                    <div className="bg-white dark:bg-black rounded-lg shadow-md p-6 mb-8">
                        ...
                        <div className="text-center py-6 text-gray-500">
                            No geographic analysis data available for these
                            keywords.
                        </div>
                    </div>
                );
                ...
                <div className="mb-8">
                    ...
                    <ResponsiveContainer width="100%" height={500}>
                        ...
                        >
                            ...
                            />
                            {Object.keys(
                                analysis.topPerformingGeos.states[0].values
                            ).map((keyword, index) => (
                                <Bar
                                    key={keyword}
                                    ...
                                    radius={[4, 4, 0, 0]}
                                >
                                    {analysis.topPerformingGeos.states.map(
                                        (entry, idx) => (
                                            <Cell
                                                key={`cell-${idx}`}
                                                fill={
    ...
    const formatDateRange = (dates: TimeData[]): string => {
        ...
        return `${formatDate(firstDate.startDate)}-${formatDate(
            lastDate.endDate
        )}`;
    };
                            ...
                            <div className="bg-white rounded-lg shadow-md overflow-hidden dark:bg-black">
                                ...
                                <div className="p-6">
                                    {isLoadingInterest ? (
                                        ...
                                        <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                                            No trend data available for the
                                            selected keywords.
                                        </div>
                                    )}
                                </div>
                            ...
                            <div>
                                ...
                                {isLoadingQuery ? (
                                    ...
                                    <div className="text-center py-10 text-gray-500 bg-white rounded-lg shadow-md">
                                        No related queries data available for
