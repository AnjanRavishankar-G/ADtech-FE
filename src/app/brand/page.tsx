...
async function fetchFilteredBrandTargetData(
  startDate: string,
  endDate: string
) {
  try {
...
export default function BrandTargetTables() {
  const [brandTargetData, setBrandTargetData] = useState<
    BrandTargetData[] | null
  >(null);
  const [uniqueBrandTargetData, setUniqueBrandTargetData] = useState<
    BrandTargetData[]
  >([]);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (startDate && endDate) {
        ...
        try {
          const filteredData = await fetchFilteredBrandTargetData(
            formattedStartDate,
            formattedEndDate
          );
  ...
  const brandProgressData = uniqueBrandTargetData.map((brand) => {
    const progress =
      brand.Target > 0 ? (brand.TargetAchieved / brand.Target) * 100 : 0;
  ...
  const combinedProgress = Math.round(
    (totalTargetAchieved / totalTarget) * 100
  );
  ...
  const brandSalesData = uniqueBrandTargetData.map(
    (brand) => brand.TargetAchieved
  );
  ...
  const brandProgressDataTop5 = topBrandsBySales.map(
    (brand) => brand.TargetAchieved
  );
                ...
                <TableBody>
                  {uniqueBrandTargetData.map((brand, index) => (
                    <TableRow key={`${brand.Brand}-${brand.DateTime}-${index}`}>
                      ...
                      <TableCell>
                        {brand.Goal?.toLocaleString() || "-"}
                      </TableCell>
                      <TableCell>{brand.DailySales}</TableCell>
                      <TableCell>
                        {brand.Target?.toLocaleString() || "-"}
                      </TableCell>
                      <TableCell>
                        {brand.Target > 0
          ...
          <div className="mt-12 flex gap-4 rounded-2xl">
            <div className="w-1/2 shadow-2xl p-4 bg-white rounded-lg dark:bg-black dark:text-white dark:shadow-[-10px_-10px_30px_4px_rgba(0,0,0,0.1),_10px_10px_30px_4px_rgba(45,78,255,0.15)]">
              ...
              <h2 className="text-2xl font-bold mb-4 mt-8 text-center ">
                Top 5 Brands Based on Sales Achieved
              </h2>
